...
from qaapy_kdpy.definition import create_obj_data
...
def read_files(args: namedtuple):
    ...
    if args.__qualname__ != 'Args':
        raise Exception('É necessário que o argumento seja um objeto do tipo Args.' + \
                            ' Para isso utilize a função qaapy_kdpy.create_args')
    else:
    ...
    data_object.coleta = pd.read_excel(
                                        args.coleta['file_name'],
                                        sheet_name = args.coleta['sheet_name'],
                                        header = 0,
                                        index_col = 0)
    
    data_object.kd_zeu = pd.read_excel(
                                        args.kd_zeu['file_name'],
                                        sheet_name = args.kd_zeu['sheet_name'],
                                        header = 0,
                                        index_col = 0)
        
    data_object.kd_3m = pd.read_excel(
                                        args.kd_3m['file_name'],
                                        sheet_name = args.kd_3m['sheet_name'],
                                        header = 0,
                                        index_col = 0)

    data_object.kd_6m = pd.read_excel(
                                        args.kd_6m['file_name'],
                                        sheet_name = args.kd_6m['sheet_name'],
                                        header = 0,
                                        index_col = 0)
